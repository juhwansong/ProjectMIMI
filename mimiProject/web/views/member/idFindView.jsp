...
<style type="text/css">
	...
	<article class="container join-content">	
		<h3>아이디 찾기</h3>
	    <hr class="margin2">
	    <div class="col-md-6 col-md-offset-3" style="margin-top:100px; margin-bottom: 50px;">
	    	<form role="form">
		    	<div class="form-group" >
		    	  <p class="help-block text-left" style="color:#5cb85c; margin-bottom: 40px;">-아이디를 찾기 위해 이메일 인증이 필요합니다.</p>	
	              <label for="username">이름</label>
	              <input type="text" class="form-control" id="username" name="username" maxlength="20" value="">
	              <input type="hidden" id="saveusername" value="">
	            </div>
	            <div class="form-group">
			...
			$(document).on("click", "#emailcert_btn", function(){ //인증하기 클릭 시 해당 이메일로 인증번호 전송  
				...
				$.ajax({
      	          	...
      	          	success : function(data){ 
      	            	if(data !== ""){
      	            		$("#saveemail").val($("#email").val().trim());//인증번호 받고 이메일과 이름 바꿔치기 방지를 위해
      	            		$("#saveusername").val($("#username").val().trim());
							...
							booleanemail = true;
      	            		...
      	            		$("#checkemail").html("이름과 이메일이 일치하지 않습니다.");
      		...
      		$(document).on("click", "#next-btn", function(){
      			...
      			$.ajax({
      	          	...
      	          	success : function(data){ 
      	            	if(data !== "" && $("#saveemail").val().trim() === $("#email").val().trim() && $("#saveusername").val().trim() === $("#username").val().trim()){
	      	            	...
	      	            	//파라미터가 유저 id이기에 post방식
	      	      			...
	      	      			form.method = "post";
	      	      			form.action = "/mimi/views/member/memberIdView.jsp"; //필요한 정보인 id를 가지고 있으니 바로 뷰로 이동
	      	      			
	      	      			document.body.appendChild(form);
	      	      			form.submit();
      	            	}
      	            	else{
      	            		alert("이름과 이메일을 다시 확인해 주세요.");         		
      	            	}
      	        	}                     
      	  		});
      		});
