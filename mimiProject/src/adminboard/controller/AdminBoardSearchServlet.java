...
public class AdminBoardSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String query = null;
		if(category.equals("C0") || category == null)//카테고리 미선택 OR 전체선택
			query = " WHERE ( TITLE LIKE '%" + keyword + "%' OR CONTENTS LIKE '%" + keyword + "%' ) ";
		else
			query = " WHERE CATEGORY_NO = '" + category + "' AND ( TITLE LIKE '%" + keyword + "%' OR CONTENTS LIKE '%" + keyword + "%' ) ";
