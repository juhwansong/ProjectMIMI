...
public class AdminBoardDeleteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(bservice.deleteAdminBoard(boardNum) > 0){
				//삭제 성공시, 첨부파일이 있을 경우
				...
				if(oldContent.contains("<img src=")){ //그림첨부를 했을때만
					...
					String newSavePath = request.getSession().getServletContext().getRealPath("resources/files/adminboard");
				...
				response.sendRedirect("/mimi/adminboardlist");
			}else{
