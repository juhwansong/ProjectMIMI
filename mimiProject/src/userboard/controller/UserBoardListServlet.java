...
public class UserBoardListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			//해당 페이지에 보이게할 목록 조회
			ArrayList<Board> list = ubservice.selectUserBoardList(currentPage, limit);
			//ArrayList<Board> list = ubservice.selectUserBoardList2();
			//총 페이지수 계산 
			...
			if(list.size() > 0){
				view = request.getRequestDispatcher(
						"views/userReview/userReviewList.jsp");
